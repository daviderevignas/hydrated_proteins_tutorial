! 			Hydrated proteins Monte Carlo simulation 

! (17.04.24)
! This program was derived from a source code provided by V. Bianco called PROTEIN_FOLDING_APRIL_2021.f90
! The following modification were applied by D. Revignas:
! 1) "num_max_aminoacids" was reduced form 21 to 20 to allow for the employment of aapot.dat 
!     file from 2016 version of the program for consistency with the paper "In Silico Evidence That Protein  
!     Unfolding is a Precursor of  Protein Aggregation"
! 2) subroutine protein_rotation() was modified to avoid unwanted flux due from rounding of the center of mass 
!     position
! 3) added saving of "status" variables for further analysis
...
module	common_variables_folding
   ...
   ! DR: Parameters of the sigmoid
   REAL(KIND=8) :: DR_HB_f, DR_HB_beta = 30.0, DR_HB_l_threshold = 2.0d0**(1.0d0/3.0d0) 
...
!DR: added this variable
   ...
   open(1133,file="output_files/hor_HB.dat",status="replace")
   open(1134,file="output_files/ver_HB.dat",status="replace")
   open(1135,file="output_files/coop_bonds.dat",status="replace")
   open(1136,file="output_files/coop_bonds_PHO.dat",status="replace")
   close(1133)
   close(1134)
   close(1135)
   close(1136)
...
!DR: print to check consistency of input
   open(10,file="output_files/output_check_1.txt",status="unknown")
   ...
   close(10)
   !DR: print to check consistency of input
   open(10,file="output_files/output_check_2.txt",status="unknown")
...
! time, energy, prot_ ene, vol, HB_bulk, HB_phob, HB_phil, HB_mix, Nim, Nim_phob, bond_phob, bond_phil, cp_ran/cp_max, cp/cp_max
   ...
   open(20,file="output_files/Data_folding.dat",status="unknown")
   open(69,file="output_files/Protein_Configurations.dat",status="unknown")

   !DR:
   open(130,file="output_files/rx.dat",status="unknown")
   open(131,file="output_files/ry.dat",status="unknown")
   do i=0,num_site-1
   ...
   close(130)
   close(131)
   open(130,file="output_files/status0.dat",status="unknown")
   open(131,file="output_files/status1.dat",status="unknown")
   open(132,file="output_files/status2.dat",status="unknown")
   open(133,file="output_files/status3.dat",status="unknown")
   open(13342,file="output_files/latt_size_parameters.dat",status="unknown")
   write(13342,*) DR_HB_beta, DR_HB_l_threshold
   close(13342)
   open(13342,file="output_files/latt_size.dat",status="unknown")
...
!***************************************
   ...
   open(25,file="output_files/Acceptance.dat",status="unknown")
   ratio=acc_rotation
...
SUBROUTINE initial_setting()
   ...
   open (87, file="output_files/Initial_Configuration.dat",status="unknown")
   do i=0, num_site-1
      x = rx(i)
      y = ry(i)
   ...
   open (87, file="output_files/Initial_Protein_Conformation.dat",status="unknown");
   do n=1,number_proteins
      do i=1,protein_length(n)
...
SUBROUTINE Bonds_DR()
   ...
   open(1133,file="output_files/hor_HB.dat",status="unknown",access="append")
   open(1134,file="output_files/ver_HB.dat",status="unknown",access="append")
   open(1135,file="output_files/coop_bonds.dat",status="unknown",access="append")
   open(1136,file="output_files/coop_bonds_PHO.dat",status="unknown",access="append")
